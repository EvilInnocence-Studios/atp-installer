...
export interface ICustomAPI {
  ...
  startDeployAllInfrastructure: (config: AppConfig) => void
  continueDeployAllInfrastructure: (shouldContinue: boolean) => void
  onDeployAllCertPending: (callback: (options: any[]) => void) => (() => void)
